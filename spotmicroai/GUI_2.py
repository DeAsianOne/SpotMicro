...
# opens window to choose motor
def open_calibration():
    global chosen_motor
    chosen_motor = motor_button.value
    motor_window.show()

# opens window for calibration
...
# function to move robot up (static)
def move_up():
    global L0
    shoulder_angle = 90
    L0 += 1
...
# function to move robot down (static)
def move_down():
    global L0
    shoulder_angle = 90
    L0 -= 1
...
# srart queue to read button during loop
# if button pressed again walk stops
def walk_queue():
    global stop_threads
    stop_threads += 1
...
# draw buttons on inital page
# when button pressed, function in command is run
# all buttons in this group part of main app
...
# new window to choose motor
calibration_window = Window(app, title="Calibration Screen", height=320, width=480, visible=False)
# options of all legs for user to choose to calibrate
# clicking on leg opens new calibration window for that leg
...
# updates motor info to user settings
